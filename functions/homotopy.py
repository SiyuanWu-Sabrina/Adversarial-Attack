...
def homotopy_attack_white(target_model, dataloader, config, **kwargs):
    ...
    for idx, data in enumerate(dataloader):
        '''数量到了就该结束了好吧'''
        print('current cnt:', counter)
        if counter == config.target_count:
            break

        if config.dataset_type == 'ImageNet':
            input, label, image_names = data
        ...
        '''
        if config.tar == 1:
            print('target = ', config.args['target_class'])
            if original_class == config.args['target_class']:
                continue
        ...
        if config.tar == 1:
            adv = homotopy(loss_type='ce', net=net, original_img=ori_image, target_class=config.args['target_class'], original_class=original_class,
                           tar=1, max_epsilon=config.args['max_epsilon'], dec_factor=config.args['dec_factor'], val_c=config.args['val_c'], val_w1=config.args['val_w1'],
                           val_w2=config.args['val_w2'], max_update=config.args['max_update'], maxiter=config.args['maxiter'], val_gamma=config.args['val_gamma'])
        elif config.tar == 0:
            adv = homotopy(loss_type='ce', net=net, original_img=ori_image, target_class=config.args['target_class'], original_class=original_class,
                           tar=0, max_epsilon=config.args['max_epsilon'], dec_factor=config.args['dec_factor'], val_c=config.args['val_c'], val_w1=config.args['val_w1'],
                           val_w2=config.args['val_w2'], max_update=config.args['max_update'], maxiter=config.args['maxiter'], val_gamma=config.args['val_gamma'])
        else:
            print('别皮......')
            exit(0)
        ...
        if cur_l0 > 0 and (config.tar == 1 and config.args['target_class'] == target.item() or config.tar == 0 and original_class != target.item()):
